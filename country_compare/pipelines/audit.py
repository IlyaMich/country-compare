...
def _write_csv(
    path: Path, rows: list[dict[str, Any]], *, dataclass_type: type[Any]
) -> None:
    dataframe = (
        pd.DataFrame(rows) if rows else _empty_dataframe_for_dataclass(dataclass_type)
    )
...
def write_audit_artifacts(
    ...
    row_counts = {
        "canonical": (
            0
            if result.canonical_dataframe is None
            else int(len(result.canonical_dataframe.index))
        ),
    ...
    validation_summary = {
        ...
        "errors": (
            ...
            else list(result.validation_report.error_messages)
        ),
        "warnings": (
            []
            if result.validation_report is None
            else list(result.validation_report.warning_messages)
        ),
        "validated_row_count": (
            0
            if result.validation_report is None
            else result.validation_report.validated_row_count
        ),
        "source_issue_count": (
            0
            if result.validation_report is None
            else result.validation_report.source_issue_count
        ),
        "rejected_row_count": (
            0
            if result.validation_report is None
            else result.validation_report.rejected_row_count
        ),
        "merge_checked": (
            False
            if result.validation_report is None
            else result.validation_report.merge_checked
        ),
        "merge_conflict_count": (
            0
            if result.validation_report is None
            else result.validation_report.merge_conflict_count
        ),
        "config_checked": (
            False
            if result.validation_report is None
            else result.validation_report.config_checked
        ),
    }
    publication_summary = {
        "attempted": (
            False
            if result.publication_report is None
            else result.publication_report.attempted
        ),
        "ok": (
            None if result.publication_report is None else result.publication_report.ok
        ),
        "row_count": (
            0
            if result.publication_report is None
            else result.publication_report.row_count
        ),
        "target_backend": (
            None
            if result.publication_report is None
            else result.publication_report.target_backend
        ),
        "target_path": (
            None
            if result.publication_report is None
            else result.publication_report.target_path
        ),
        "wrote_metric_dataset": (
            False
            if result.publication_report is None
            else result.publication_report.wrote_metric_dataset
        ),
        "error": (
            None
            if result.publication_report is None
            else result.publication_report.error
        ),
    }
    merge_summary = {
        "attempted": (
            False if result.merge_report is None else result.merge_report.attempted
        ),
        "ok": None if result.merge_report is None else result.merge_report.ok,
        "duplicate_key_conflict_count": (
            0
            if result.merge_report is None
            else result.merge_report.duplicate_key_conflict_count
        ),
        "duplicate_key_row_count": (
            0
            if result.merge_report is None
            else result.merge_report.duplicate_key_row_count
        ),
        "conflict_keys_preview": (
            []
            if result.merge_report is None
            else list(result.merge_report.conflict_keys_preview)
        ),
    ...
    run_summary = {
        ...
        "merge": merge_summary,
        "run_metadata": (
            None if result.run_metadata is None else asdict(result.run_metadata)
        ),
    }
    run_summary_path = output_dir / "run_summary.json"
    run_summary_path.write_text(
        json.dumps(run_summary, indent=2, default=_json_default), encoding="utf-8"
    )
    ...
    for source_result in result.source_results:
        ...
        rejected_rows.extend(
            asdict(rejected) for rejected in source_result.rejected_rows
        )
    ...
    publication_summary_path.write_text(
        json.dumps(publication_summary, indent=2, default=_json_default),
        encoding="utf-8",
    )
    ...
    if result.canonical_dataframe is not None:
        ...
        result.canonical_dataframe.head(
            max(1, int(request.canonical_preview_rows))
        ).to_csv(canonical_preview_path, index=False)
        artifact_paths["canonical_preview"] = str(canonical_preview_path)

    if (
        result.merge_report is not None
        and result.merge_report.conflict_dataframe is not None
        and not result.merge_report.conflict_dataframe.empty
    ):
