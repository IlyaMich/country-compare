...
class CanonicalTabularPassthroughAdapter(SourceAdapter):
    def process(self, assets: list[Any], *, source_spec: Any | None = None) -> pd.DataFrame:
        ...
        if len(assets) != 1:
            raise ValueError(f'passthrough adapter expects exactly one asset, received {len(assets)}')
        asset = assets[0]
    ...
    def to_standardized_dataframe(self) -> pd.DataFrame:
        ...
        return self._normalize_to_canonical_columns(dataframe, source_spec=self.current_source_spec)

    def _normalize_to_canonical_columns(self, dataframe: pd.DataFrame, *, source_spec: Any | None) -> pd.DataFrame:
        ...
        if column_mapping:
            frame = frame.rename(columns={self._normalize_column_name(source_column): target_column for source_column, target_column in column_mapping.items()})
        ...
        if missing_required:
            raise ValueError(f'passthrough adapter could not produce required canonical columns: {missing_required}')
...
register_source_adapter(CANONICAL_TABULAR_PASSTHROUGH_ADAPTER_ID, CanonicalTabularPassthroughAdapter, description='Canonical or near-canonical CSV/Parquet passthrough adapter.', replace=True)
