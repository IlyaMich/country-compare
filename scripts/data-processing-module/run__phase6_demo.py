...
def main() -> None:
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        print(
            "validation ok:",
            result.validation_report.ok if result.validation_report else None,
        )
        print(
            "publication ok:",
            result.publication_report.ok if result.publication_report else None,
        )
        print(
            "audit written:",
            result.audit_report.written if result.audit_report else None,
        )
        print(
        ...
        checks = {
            ...
            "remote_asset_materialized": len(assets) == 1
            and assets[0].local_path.exists(),
            "remote_mode_tracked": bool(
                assets[0].metadata.get("acquisition_mode") == "remote_pull"
            ),
            "manifest_remote_success_ok": result.ok,
            "audit_written": bool(result.audit_report and result.audit_report.written),
            "publication_wrote_rows": bool(
                store.written is not None and len(store.written.index) == 2
            ),
        }
