...
def verify_bearer_token(authorization: str | None, settings: ServiceSettings) -> None:
    ...
    if not authorization or scheme.lower() != "bearer" or not token:
        metrics.record_auth_failure(reason="missing_bearer_token")
        raise ServiceError(
            "unauthorized",
    ...
    if not settings.service_token:
        metrics.record_auth_failure(reason="service_token_not_configured")
        raise ServiceError(
    ...
    if not hmac.compare_digest(token, settings.service_token):
        metrics.record_auth_failure(reason="invalid_bearer_token")
        raise ServiceError(
            "unauthorized",
