...
def test_pipeline_run_fails_for_invalid_canonical_input(tmp_path: Path) -> None:
    ...
    detailed_errors = (
        list(result.validation_report.error_messages)
...
def test_pipeline_surfaces_config_validation_failure(
    ...
    assert any(
        "config/dataframe mismatch" in message
        for message in result.validation_report.error_messages
    )
...
def test_pipeline_fails_when_merged_sources_create_duplicate_primary_keys(
    tmp_path: Path,
) -> None:
    raw = pd.DataFrame(
        {
...
def test_pipeline_reports_source_aware_merge_conflicts(
    tmp_path: Path, canonical_dataframe: pd.DataFrame
) -> None:
    ...
    assert set(result.merge_report.conflict_dataframe["_source_id"].tolist()) == {
        "source_a",
        "source_b",
    }
    assert "duplicate canonical primary-key rows detected after merge" in (
        result.error or ""
    )
...
def test_pipeline_manifest_path_supports_remote_url_sources(
    tmp_path: Path, canonical_dataframe: pd.DataFrame
) -> None:
