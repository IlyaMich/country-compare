...
class WideYearMetricCsvAdapter(SourceAdapter):
    ...
    def to_standardized_dataframe(self) -> AdapterResult:
        if self.current_asset is None:
            raise ValueError(
                "no current asset was prepared for the wide_year_metric_csv adapter"
            )
    ...
    def _adapt_dataframe(
        ...
        if country_name_column is None or country_code_column is None:
            ...
            raise ValueError(
                f"wide_year_metric_csv adapter could not resolve required raw columns: {missing}"
            )
        ...
        if not year_columns:
            raise ValueError(
                "wide_year_metric_csv adapter could not detect any year columns"
            )
        long_df = frame.loc[
        ...
        for idx in long_df.index[blank_country_mask].tolist():
            issues.append(
                RowIssue(
                    ...
                    adapter_id=getattr(
                        source_spec, "adapter_id", WIDE_YEAR_METRIC_CSV_ADAPTER_ID
                    ),
        ...
        long_df[country_name_column] = (
            long_df[country_name_column].astype("string").str.strip()
        )
        long_df[country_code_column] = (
            long_df[country_code_column].astype("string").str.strip().str.upper()
        )
        ...
        for idx in long_df.index[invalid_year_mask].tolist():
            issues.append(
                RowIssue(
                    ...
                    message=(
                        f"row was dropped because year label could not be parsed: "
        ...
        if missing_required:
            raise ValueError(
                f"wide_year_metric_csv adapter could not produce required canonical columns: "
                f"{missing_required}"
            )
        required_metadata = [
            "metric_id",
        ...
        if missing_metadata:
            raise ValueError(
                f"wide_year_metric_csv adapter requires SourceSpec metadata for: "
