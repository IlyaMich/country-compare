...
class PipelineEngine:
    ...
    def run(self, request: ProcessingRequest) -> ProcessingResult:
        ...
        run_metadata = RunMetadata(
            ...
            source_count=sum(1 for s in request.sources if s.enabled),
        )
        ...
        try:
            ...
            if not merge_report.ok or merged is None:
                ...
                validation_report = ValidationReport(
                    ...
                    rejected_row_count=sum(
                        item.rejected_row_count for item in source_results
                    ),
                    merge_checked=True,
                    merge_conflict_count=merge_report.duplicate_key_conflict_count,
                )
                raise CanonicalValidationError(message)
            validation_report, canonical_dataframe, metric_dataset = (
                self._validate_merged_dataframe(
                    merged,
                    request=request,
            ...
            if not validation_report.ok:
                raise CanonicalValidationError(
                    "; ".join(validation_report.error_messages) or "validation failed"
                )
            if request.publish:
        ...
        if validation_report is None:
            validation_report = ValidationReport(
                ...
                merge_checked=merge_report.attempted,
                merge_conflict_count=merge_report.duplicate_key_conflict_count,
            )
        ...
        result = ProcessingResult(
            ...
            metric_dataset=(
                metric_dataset
                if (run_error is None and request.write_metric_dataset)
                else None
            ),
    ...
    def _process_source(
        self, source_spec: Any, *, request: ProcessingRequest
    ) -> SourceProcessingResult:
        try:
            assets = tuple(
                self.acquirer.acquire(
                    source_spec, raw_root=self._resolve_raw_root(request.raw_root)
                )
            )
            ...
            if raw_row_count is None:
                raw_row_count = int(len(output.dataframe.index)) + len(
            ...
            return SourceProcessingResult(
                ...
                canonical_row_count=int(len(output.dataframe.index)),
                issues=output.issues,
                rejected_rows=output.rejected_rows,
                warnings=output.warnings,
                tags=tuple(getattr(source_spec, "tags", ()) or ()),
                labels=dict(getattr(source_spec, "labels", {}) or {}),
            )
        except Exception as exc:
            return SourceProcessingResult(
                source_id=source_spec.source_id,
    ...
    def _execute_adapter(
        adapter: Any, assets: tuple[Any, ...], *, source_spec: Any
    ) -> AdapterResult:
        try:
    ...
    def _merge_source_results(
        source_results: list[SourceProcessingResult],
    ) -> tuple[MergeReport, pd.DataFrame | None]:
        frames = [
        ...
        duplicate_mask = (
            merged_with_lineage.duplicated(subset=list(PRIMARY_KEY_COLUMNS), keep=False)
            if not merged_with_lineage.empty
        ...
        if not merged_with_lineage.empty and duplicate_mask.any():
            ...
            preview_columns = [
                column
                for column in [*PRIMARY_KEY_COLUMNS, "_source_id", "_adapter_id"]
                if column in conflicts.columns
            ]
            preview_rows = (
                conflicts.loc[:, preview_columns]
                .drop_duplicates()
                .head(20)
            ...
            return (
                MergeReport(
                    ...
                    input_row_count=sum(len(frame.index) for frame in frames),
                    merged_row_count=int(len(merged.index)),
                    duplicate_key_conflict_count=int(
                        conflicts.loc[:, list(PRIMARY_KEY_COLUMNS)]
                        .drop_duplicates()
                        .shape[0]
                    ),
                    duplicate_key_row_count=int(len(conflicts.index)),
                    conflict_keys_preview=tuple(preview_rows),
                    conflict_dataframe=conflicts,
                    error=message,
                ),
                None,
            )
    ...
    def _validate_merged_dataframe(
        ...
        *,
        ...
        try:
            validated_dataframe, metric_dataset = self._prepare_dataframe_for_storage(
                dataframe
            )
        except Exception as exc:
        ...
        if not errors and request.validate_against_config:
            ...
            if metrics_config is None:
                ...
                try:
                    ...
                    validate_metrics_against_dataframe(
                        metrics_config, validated_dataframe
                    )
                except Exception as exc:
    ...
    def _prepare_dataframe_for_storage(
        ...
        for function_name in (
            ...
            if extracted is not None:
                ...
                break
        dataset = None
        dataframe_to_metric_dataset = getattr(
            validation_module, "dataframe_to_metric_dataset", None
        )
    ...
    def _finalize_run_metadata(
        ...
        *,
        ...
        issue_count = (
            sum(item.issue_count for item in source_results)
            + validation_report.issue_count
        )
        warning_count = len(warnings) + len(validation_report.warning_messages)
        error_count = sum(item.error_count for item in source_results) + len(
            validation_report.error_messages
        )
        if error:
            error_count += 1
        return replace(
            ...
            canonical_row_count=(
                0
                if canonical_dataframe is None
                else int(len(canonical_dataframe.index))
            ),
            rejected_row_count=sum(item.rejected_row_count for item in source_results),
    ...
    def _attach_audit_report(
        ...
        if output_dir is None:
            output_dir = (
                Path(next(iter(artifact_paths.values()))).parent
                if artifact_paths
                else None
            )
...
def _coerce_adapter_output(output: Any) -> AdapterResult:
    if isinstance(output, pd.DataFrame):
        return AdapterResult(
            dataframe=output.copy(deep=True), raw_row_count=int(len(output.index))
        )
...
def _coerce_issues(values: Any) -> list[RowIssue]:
    ...
    for value in values or []:
        ...
        raise AdapterExecutionError(
            "adapter issues must contain RowIssue objects or mappings"
        )
    return issues
...
def _fallback_validate_dataframe(dataframe: pd.DataFrame) -> None:
    missing_required = [
        column for column in ALL_COLUMNS[:11] if column not in dataframe.columns
    ]
    if missing_required:
        raise CanonicalValidationError(
    ...
    if duplicate_mask.any():
        ...
        raise CanonicalValidationError(
            ...
            + str(duplicates.to_dict(orient="records"))
        )
    for column in [
        "country_code",
        "country_name",
