...
class AppFacade:
    ...
    def get_overview_status(
        ...
        config_status = self.config.get_status(
            validate_against_dataset=validate_config_against_dataset
            and dataset_summary.exists
        )

        warnings: list[str] = []
        if not dataset_summary.exists:
            warnings.append(
                "No dataset is currently available, so comparison pages cannot run yet."
            )
        if not config_status.validation.valid:
            warnings.append(
                "Configuration is not currently valid, so scoring/comparison flows should remain "
                "disabled."
            )
        if dataset_summary.exists and not validate_config_against_dataset:
            warnings.append(
                "Dataset-aware config validation is currently skipped. Enable it from the overview "
                "page when needed."
            )
    ...
    def get_overview_status_payload(
        self, *, validate_config_against_dataset: bool = False
    ) -> dict:
