...
class PathSettings:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, "metrics_config_path", _coerce_path(self.metrics_config_path)
        )
        object.__setattr__(
            self, "scoring_config_path", _coerce_path(self.scoring_config_path)
        )
        object.__setattr__(
            self, "store_backend", str(self.store_backend).strip().lower()
        )
...
class UISettings:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, "app_title", str(self.app_title).strip() or DEFAULT_UI_APP_TITLE
        )
        object.__setattr__(
            self, "page_title", str(self.page_title).strip() or DEFAULT_UI_PAGE_TITLE
        )
        object.__setattr__(self, "page_icon", str(self.page_icon))
        object.__setattr__(
            self, "layout", str(self.layout).strip() or DEFAULT_UI_LAYOUT
        )
        object.__setattr__(
            self,
            "default_page",
            str(self.default_page).strip() or DEFAULT_UI_DEFAULT_PAGE,
        )
...
class PredictionSettings:
    ...
    def __post_init__(self) -> None:
        ...
        fallback = (
            None if self.fallback_method is None else str(self.fallback_method).strip()
        )
        ...
        object.__setattr__(
            self, "moving_average_window_size", int(self.moving_average_window_size)
        )
